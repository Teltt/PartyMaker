@tool
@icon("res://Resource/Graphics/BluePassSpace.png")
extends Space
class_name BluePassSpace
...
func activate(activation_type: int, pl: Player = null) -> Result:
	if activation_type == Define.ACTIVATION.ACT_PASS || activation_type == Define.ACTIVATION.ACT_EVENT_PASS:
